...
describe("TokenSwap", function () {
    ...
    it("Should revert if token is the same", async function () {
        await guzToken
            .connect(addr1)
            .approve(tokenSwap, ethers.parseUnits("100", 18));

        // Expect the error CanNotSwapSameToken when creating order for the same token
        await expect(
            tokenSwap
                .connect(addr1)
                .createOrder(
        ...
        ).to.be.revertedWithCustomError(tokenSwap, "CanNotSwapSameToken");
    });
    ...
    it("Should revert if non-owner tries to approve the order", async function () {
        ...
        // Expect the error OnlyOwnerCanPerformThisAction when a non-owner tries to approve the order
        await expect(
